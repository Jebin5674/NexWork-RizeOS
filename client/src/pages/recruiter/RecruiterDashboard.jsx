...
const RecruiterDashboard = () => {
  ...
  const fetchMyJobs = async () => {
    try {
        ...
        if (res.data.success) {
            ...
            if (user.email) {
                const myJobs = allJobs.filter(job => job.recruiterEmail === user.email);
                setJobs(myJobs);
            }
        }
    } catch (error) {
  ...
  const styles = {
    ...
    postBtn: { backgroundColor: '#D97706', color: 'white', padding: '12px 24px', borderRadius: '10px', border: 'none', fontWeight: '800', cursor: 'pointer', display: 'flex', alignItems: 'center', boxShadow: '0 4px 10px rgba(217, 119, 6, 0.4)' },
